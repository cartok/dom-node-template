...
const createDocumentFragment = window !== undefined && window.document !== undefined && window.document.createRange !== undefined ? tagText => window.document.createRange().createContextualFragment(tagText) : tagText => {
    const parser = new window.DOMParser();
...
export default class NodeTemplate {
    ...
    constructor(tagText, options) {
        ...
        const createReferences = (() => {
            ...
            // add root reference
            this.root = this.fragment.childNodes.length === 1 ? this.fragment.firstElementChild : Array.from(this.fragment.childNodes);
        })();
    }
    /**
    ...
    getNode(query, options) {
        ...
        switch (getQueryType(query)) {
            ...
            case "class":
            case "query":
                return options.firstMatch === true ? this.fragment.querySelector(query) : this.fragment.querySelectorAll(query);
            default:
 ...
 * @param {*} getTagContent 
 */
function getTagGroupByName(text, tagName, getTagContent) {
    // console.log("-----------------------------------------")
...
function handleTagGroup(tagGroup, options) {
...
function getQueryType(query) {
    return query instanceof Node ? "Node" : query.charAt(0) === "." ? "id" : query.charAt(0) === "#" ? "class" : "query";
}